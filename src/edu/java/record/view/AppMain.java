...
public class AppMain {
	...
	private Calendar calMain = new GregorianCalendar();
	private Calendar calMini = new GregorianCalendar();
...
//		panleMini.setBackground(new Color(204, 204, 255));
	...
	//Main clalendar
		...
		private void setMianCal() {
			int year = calMain.get(Calendar.YEAR);
			int month = calMain.get(Calendar.MONTH) +1;
			calMain.set(year, month -1, 1);
			int startDay = calMain.get(Calendar.DAY_OF_WEEK);
			startDay--;
			
			for(int i = 1; i <= calMain.getActualMaximum(Calendar.DATE); i++) {
...
//						day.setBorderPainted(true);
	...
	private void updateMonth() {
		calMini.set(Calendar.DAY_OF_MONTH, 1);
		 
	    String month = calMini.getDisplayName(Calendar.MONTH, Calendar.LONG, Locale.KOREA);
	    int year = calMini.get(Calendar.YEAR);
	    labelMini.setText(year + "년 " + month);
	    
		int startDay = calMini.get(Calendar.DAY_OF_WEEK);
		// 월이 시작하는 위치
		int numberOfDays = calMini.getActualMaximum(Calendar.DAY_OF_MONTH);
		// 해당 월의 최대 날짜
		int weeks = calMini.getActualMaximum(Calendar.WEEK_OF_MONTH);
		// 해당 월의 최대 주
